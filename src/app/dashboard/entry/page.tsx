...
export default function EntryPage() {
  ...
  useEffect(() => {
    if (cancellations > 0) {
      // Initialize cancel details with one row
      if (cancelDetails.length === 0) {
        setCancelDetails([
          { count: cancellations, bookedYear: year, bookedMonth: month, bookedHalf: half },
        ]);
      }
    } else {
      setCancelDetails([]);
  ...
  const addCancelDetail = () => {
    setCancelDetails([
      ...cancelDetails,
      { count: 0, bookedYear: year, bookedMonth: month, bookedHalf: 1 },
    ]);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (cancellations > 0 && totalCancelCount !== cancellations) {
      setMessage(
        `Cancellation details must add up to ${cancellations}. Current total: ${totalCancelCount}`
      );
    ...
    <div className="max-w-3xl">
      <div className="mb-6">
        ...
        <p className="text-slate-500">
          Enter site visits, bookings and cancellations for a 15-day period
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          <h3 className="text-sm font-semibold text-slate-900 mb-4">Select Period</h3>
          ...
          {session?.user.role === "admin" && (
            <div className="mt-4">
              ...
              <select
                ...
                className="w-full px-3 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                <option value="">Select a manager (or yourself)</option>
                {users.map((u) => (
        ...
        {/* Numbers Input */}
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          <h3 className="text-sm font-semibold text-slate-900 mb-4">Enter Data</h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">
                Site Visits
              ...
              <input
                ...
                min={0}
                value={siteVisits}
                onChange={(e) => setSiteVisits(parseInt(e.target.value) || 0)}
                className="w-full px-3 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
            <div>
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Specify when these cancelled units were originally booked
                </p>
              </div>
              <button
                type="button"
                onClick={addCancelDetail}
                ...
                <div key={index} className="flex items-end gap-3 p-3 bg-slate-50 rounded-lg">
                  <div className="flex-1">
                    ...
                    <input
                      ...
                      value={detail.count}
                      onChange={(e) =>
                        updateCancelDetail(index, "count", parseInt(e.target.value) || 0)
                      }
                  ...
                  <div className="flex-1">
                    ...
                    <select
                      value={detail.bookedYear}
                      onChange={(e) =>
                        updateCancelDetail(index, "bookedYear", parseInt(e.target.value))
                      }
                      className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                      {[2024, 2025, 2026, 2027].map((y) => (
                  ...
                  <div className="flex-1">
                    ...
                    <select
                      value={detail.bookedMonth}
                      onChange={(e) =>
                        updateCancelDetail(index, "bookedMonth", parseInt(e.target.value))
                      }
                      className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                      {Array.from({ length: 12 }, (_, i) => (
                  ...
                  <div className="flex-1">
                    ...
                    <select
                      value={detail.bookedHalf}
                      onChange={(e) =>
                        updateCancelDetail(index, "bookedHalf", parseInt(e.target.value))
                      }
                      className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                      <option value={1}>1st to 15th</option>
                  ...
                  {cancelDetails.length > 1 && (
                    <button
                      type="button"
                      onClick={() => removeCancelDetail(index)}
        ...
        <button
          type="submit"
          disabled={submitting}
          className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white font-medium py-2.5 px-6 rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed"
        >
